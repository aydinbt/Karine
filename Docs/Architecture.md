...
- `Assets/Bube/Runtime/UI/KarineUI.cs` — kit bileşenleri: `Button_` (Primary/Secondary/Ghost/Danger + basılı/devre dışı durumları), `IconButton`, `Icon`, `Panel`, `Row`, `Rule`, `Title`/`Subtitle`/`Body_`/`Technical`, `Badge`, `Dot`, `Tabs`, `DocumentNav`, `Modal`, `Notification`, `Progress`, `Tooltip`, `Radio`, `Meter`, `Counter`, `CinematicControls`, `Clock`, `Border`/`Round`/`Enter`. Prefab yoktur; "bileşen" burada `VisualElement` döndüren üretici demektir.
